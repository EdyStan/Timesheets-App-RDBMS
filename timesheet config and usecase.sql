-- EMPLOYEES table: stores user info and JSON preferences.
...
-- master list of employees; JSON prefs allows future extensibility.
/

-- Index on last_name for quick lookup
CREATE INDEX idx_employees_lastname ON employees(last_name);
 
-- PROJECTS table: list of active/inactive projects.
...
-- tracks projects; status enforces permitted values.
/

-- TIMESHEET_ENTRIES table: daily log of hours per project.
...
-- one row per employee/project/day; JSON notes for flexibility;
...
-- VIEW: per-employee total hours over all time
...
-- quick summary of total hours per employee.

-- MATERIALIZED VIEW: total hours by project, refreshed on demand
...
-- pre-aggregates project hours for high-performance reporting.

-- ========================================================
-- Sample Data Insertion & Use Case
-- ========================================================
...
-- two sample employees with JSON prefs.
...
-- two projects; second has no end_date yet.
...
-- Total hours per project (GROUP BY)
...
ORDER BY total_hours DESC;
-- Manager sees 'Website Redesign' has X hours and can reallocate resources.

-- List all employees, even those with no entries (LEFT JOIN)
-- Helps identify people who haven’t logged any time.
SELECT
  e.employee_id,
...
-- Admin can remind those with zero hours to fill in their timesheets.

-- Weekly ranking of employee total hours (ANALYTIC: RANK)
...
WHERE hrs_rank <= 5;
-- Recognize top performers or flag potential overtime.
